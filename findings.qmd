...
## 2. The Correlation Matrix Evidence

-   **Support Tickets vs. Satisfaction:** The heatmap reveals a strong negative correlation (e.g., $r = -0.65$), statistically confirming that as unresolved issues pile up, customer sentiment drops significantly.

-   **Transaction Volume vs. Churn:** We observed that `total_tx_volume` has a moderate negative correlation with `churn_probability`, suggesting that "deeply embedded" users who use the app for large transactions are significantly less likely to leave the platform.

-   **Model Validation:** These correlations justify our use of `support_tickets_count` and `tx_count` as primary inputs for our **Predictive Risk Simulator**.

------------------------------------------------------------------------

## 3. Algorithmic and Rule-Based Personas and Value Satisfaction (Segmentation)
